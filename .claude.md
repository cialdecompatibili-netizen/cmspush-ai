...
### ✅ METODO PREFERITO — Tool `github` integrato in Claude (da sessione 2026-06-08)

**NON usare più PowerShell per i push.** Claude ha un tool GitHub integrato e autenticato, più pulito e affidabile.

#### Tool disponibili (caricarli con `tool_search("github push commit")` se non già caricati):

| Tool | Quando usarlo |
|------|---------------|
| `github:push_files` | Push di più file in un solo commit |
| `github:create_or_update_file` | Push di un singolo file |
| `github:list_commits` | Verificare stato commit recenti |
| `github:get_file_contents` | Leggere un file dal repo remoto |

#### Workflow standard con il connettore:

**1. Leggi il file corrente (per ottenere SHA):**
```
github:get_file_contents(owner="cialdecompatibili-netizen", repo="cmspush", path="percorso/file.ext")
```

**2. Pusha il file modificato:**
```
github:create_or_update_file(
  owner="cialdecompatibili-netizen",
  repo="cmspush",
  branch="main",
  path="percorso/file.ext",
  message="Descrizione modifica",
  content="...contenuto...",
  sha="SHA_dal_passo_1"
)
```

**3. Push multipli file in un commit:**
```
github:push_files(
  owner="cialdecompatibili-netizen",
  repo="cmspush",
  branch="main",
  message="Descrizione",
  files=[{path: "file1.ext", content: "..."}, {path: "file2.ext", content: "..."}]
)
```

**4. Verifica commit andato a buon fine:**
```
github:list_commits(owner="cialdecompatibili-netizen", repo="cmspush", perPage=3)
```

#### Fallback PowerShell (solo se il connettore non è disponibile):
```powershell
cd "C:\Users\mirco\Desktop\cmspush"; git add .; git commit -m "descrizione modifica"; git push origin HEAD:main
```

**Regola:** dopo ogni modifica, Claude pusha autonomamente senza aspettare che Mirco lo chieda.

## Strumenti disponibili per Claude

| Strumento | Cosa fa |
|---|---|
| `github:push_files` | ✅ Push multipli file su GitHub (PREFERITO per push) |
| `github:create_or_update_file` | ✅ Push singolo file su GitHub |
| `github:list_commits` | Verifica commit recenti |
| `github:get_file_contents` | Legge file direttamente dal repo remoto |
| `Desktop Commander:read_file` | Legge file locali con offset/length (file lunghi) |
| `Desktop Commander:edit_block` | Modifica chirurgica locale con old_string/new_string |
| `Filesystem:read_multiple_files` | Legge file dal PC di Mirco |
| `Windows-MCP:PowerShell` | Comandi PowerShell (solo se necessario, non per push) |
...
## 🚀 REGOLA PUSH — Come pushare (sessione 2026-06-08)

**Criterio:** scegli in base alla dimensione del contenuto da trasferire.

| Situazione | Metodo | Perché |
|---|---|---|
| File piccolo / modifica testuale (<20KB) | `github:create_or_update_file` | Il connettore accetta stringhe brevi — veloce, zero git |
| File grande (>20KB, es. admin/index.html ~80KB) | PowerShell git | Il connettore non regge payload enormi — git push diretto |
| Push multipli file in un commit | `github:push_files` | Un solo commit, più pulito |

### PowerShell — sintassi corretta (Windows)
```powershell
# ✅ Usare ; non && in PowerShell
cd "C:\Users\mirco\Desktop\cmspush"; git add percorso/file.ext; git commit -m "msg"; git push origin HEAD:main

# Se HEAD è detached (es. dopo checkout):
git push origin HEAD:main

# Se repo locale è indietro rispetto al remoto:
git fetch origin; git push origin HEAD:main --force-with-lease
```

### Workflow completo per file grandi
1. `Desktop Commander:edit_block` — modifica chirurgica sul file locale
2. Verifica con `Desktop Commander:read_file` sul range modificato
3. `Desktop Commander:start_process` con git per pushare
...
### ✅ FATTO (2026-06-08) — Tab Articoli e Pagine stile WordPress
- Vista lista di default con bottone "✚ Nuovo articolo" / "✚ Nuova pagina"
- Modifica/Nuovo aprono vista editor (lista sparisce), bottone "← Torna alla lista"
- Dopo pubblicazione/salvataggio torna lista automaticamente dopo 1.2s
- Pagine: campo Permalink visibile solo su nuova pagina, nascosto in modifica
- IDs HTML usati: `art-lista-view`, `art-editor-view`, `pag-lista-view`, `pag-editor-view`
- Funzioni JS: `apriNuovoArticolo()`, `tornaListaArticoli()`, `apriNuovaPagina()`, `tornaPagine()`
- Commit: aad6c92

### ⚠️ ATTENZIONE — NON confondere i due progetti
- **cmspush** (questo file): `C:\Users\mirco\Desktop\cmspush` → repo `cialdecompatibili-netizen/cmspush` → admin su `/cmspush/admin/`
- **Jekyll + GitHub** (altro progetto): `C:\Users\mirco\Desktop\Jekyll + GitHub` → repo diverso → admin su `/admin/`
- I due progetti hanno entrambi `admin/index.html` ma sono file SEPARATI. Non modificare mai quello sbagliato.
...
| 2026-06-08 | Integrazioni: card strumenti automatici con link sitemap/rss dinamici | admin/index.html | cc340f8 |
| 2026-06-08 | Regola push: connettore per file piccoli, PowerShell git per file grandi (>20KB) | CLAUDE.md | — |
